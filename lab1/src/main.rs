...
fn main() {
    ...
    let accounts: Mutex<Vec<Arc<Mutex<Account>>>> = Mutex::new(Vec::new());
    ...
    for (_, line) in reader.lines().enumerate() {
        ...
        accounts.lock().unwrap().push(Arc::new(Mutex::new(Account::new(
            String::from(vec_line[0]),
    ...
    let threads_per_run = 1000;
    let accounts_len = accounts.lock().unwrap().len();
    ...
    while !is_finished {
        ...
        if command == "start" {
            ...
            for index in 0..threads_per_run {
                ...
                let account_from_ref = Arc::clone(&accounts.lock().unwrap()[index_from]);
                let account_to_ref = Arc::clone(&accounts.lock().unwrap()[index_to]);
                let current_transaction_number_ref = Arc::clone(&current_transaction_number);

                threads.push(std::thread::spawn(move || {
                    let mut amount = generate_amount();

                    let mut account_from_guard;
                    let mut account_to_guard;

                    if index_from < index_to {
                        account_from_guard = account_from_ref.lock().unwrap();
                        account_to_guard = account_to_ref.lock().unwrap();
                    } 
                    else {
                        account_to_guard = account_to_ref.lock().unwrap();
                        account_from_guard = account_from_ref.lock().unwrap();
                    ...
                    if account_from_guard.amount == 0 {
                        return;
                    }
                    ...
                    let log = transaction_number.to_string()
                        + ":"
                        + &account_from_guard.id.to_string()
                        + "->"
                        + &amount.to_string()
                        + "->"
                        + &account_to_guard.id.to_string();
                    *transaction_number += 1;
                    ...
                    println!(
                        "{} from {} to {} with log: {}",
                        amount, account_from_guard.iban, account_to_guard.iban, log
                    );
                }));

                if index % 3 == 0 {
                    for account in &*accounts.lock().unwrap() {
                        let account_ref = Arc::clone(account);
                        let account_guard = account_ref.lock().unwrap();

                        verify(
                            &account_guard,
                            account_guard.init_amount,
                            account_guard.amount,
                        );
                    }
                }
            }
